...
# nmsVec=function(x){x=paste("c('",paste(names(x),collapse="','"),"')",sep="");writeClipboard(x);return(x)}
		...
		if(version_rnx=="3.05" | version_rnx==3.05){
			...
			if(!is.na(dir_rinex) | length(path_rinex)>1){
				...
				mapply(
					...
					, path_out = files_out
					, MoreArgs = list( start_epoch=start_epoch, finish_epoch=finish_epoch , ignore.short.file=ignore.short.file)
						)
	...
	.rinex305_clip=function(
		...
		if(is.na(path_out)) path_out = file.path(dir_out, basename(path_rinex))
		...
		idx_finish = ifelse(length(epochs_in) > finish_epoch, epochs_in[finish_epoch] , 
