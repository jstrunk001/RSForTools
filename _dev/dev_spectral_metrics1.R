...
#
  ...
  if(!is.na(outlier[1])) outlier_ids = z < outlier[1] | z > outlier[2]
  if(is.na(outlier[1])) outlier_ids = rep(F,length(z))

  #index values above z cover threshold and omit outliers based on z
  idx_z = (z >= htcover) & (!outlier_ids )

  #compute intensity and spectral quantiles
  #MUST BE A LIST
  mets_in = c(
      list()
    , n_z=as.numeric(length(z))
    , n_z1 = as.numeric(sum(idx_z))
    #, .f_qt(i,qts,"i" ,idx_z )
    , .f_qt(r,qts,"r" ,idx_z )
    , .f_qt(g,qts,"g" ,idx_z )
    , .f_qt(b,qts,"b" ,idx_z )
    , .f_qt(nir,qts,"nir" ,idx_z )
      )
  return(mets_in)

}

  #function to filter, compute quantiles, name vector
  .f_qt = function(x,qts,nm,idx){

      #subset values based on supplied index - usually based on z
      x1 = x[idx]
      #test for bad vector
      bad_x = length(x1) < 5 | diff(range(x1))==0
      if(bad_x) qts_i = rep(0.0,length(qts))
      #compute quantiles
