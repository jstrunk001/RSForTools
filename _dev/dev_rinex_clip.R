...
# nmsVec=function(x){x=paste("c('",paste(names(x),collapse="','"),"')",sep="");writeClipboard(x);return(x)}
    ...
    end_header = which(grepl(".*END OF HEADER",rinex_in))
    header_in = rinex_in[1:end_header]
    obs_in = rinex_in[-(1:end_header)]
    year2 = ""

		#find each epoch
		epochs_in = which(!grepl(paste0("^  ",".*"),obs_in[-(1:end_header)]))

		#use last epoch in the case
		if(is.na(finish_epoch)) finish_epoch = nrow(obs_in)
		...
		#actually slice
		rin_clip = c(header_in, obs_in[idx_start:idx_finish])
