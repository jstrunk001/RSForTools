...
#
  ...
  if(!is.na(x[1]) & !is.na(y[1]) & !is.na(z[1])){
    ...
    mets_in[paste0("zCover_",htcover)] = round(100*length(z1)/ length(z),2)
    ...
    #volume and area metrics
      #all points
      mets_in[["xyAreaAll"]] = nrow(combnsxy) * resxy^2
      #points above threshdol
      mets_in[["xyArea"]] = nrow(combnsxy1) * resxy^2
      #rescale using over ht threshold to all
      mets_in["areaRat"] = round(100*mets_in[["xyArea"]] / mets_in[["xyAreaAll"]],2)
